...
from db import (
    ...
    reject_ticket_db, get_pending_withdrawals,
...
class SubscriptionMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data: dict):
        ...
        # Если уже подписан в базе — сразу пропускаем
...
async def card_type_selected(call: CallbackQuery, state: FSMContext):
    ...
    names = {"tbank": "Т-Банк", "alfa": "Альфа-Банк", "sber": "СберБанк", "other": "Другая карта"}
    name = names.get(card_type, "Карта")
    await call.message.edit_text(
...
async def done_callback(call: CallbackQuery, state: FSMContext):
    ...
    if t[2] == "CARD":
        await call.message.edit_text("💰 Введите сумму для начисления пользователю:")
        await state.set_state(TicketStates.waiting_card_price)
        await state.update_data(ticket_id=tid)
        return

    amount = 4.4 if t[2] == "MAX" else 0
...
async def card_price_input(message: Message, state: FSMContext):
    try:
        ...
        complete_ticket(tid, amount)
        await message.answer(f"✅ Карта #{tid} оплачена на {amount} USDT.")
    except:
...
async def admin_cancel(call: CallbackQuery):
    ...
    t = get_ticket(tid)
    if t and t[1]:
        await bot.send_message(t[1], f"❌ Заявка #{tid} была отклонена.")
...
async def ask_code(call: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(
        t[1], 
        "📨 На ваш номер MAX отправлен SMS-код.\nВведите 6-значный код сюда:",
        reply_markup=back_to_main()
    )
...
async def code_input(message: Message, state: FSMContext):
    ...
    if tid:
        ...
        await message.answer("✅ Код успешно отправлен администратору!")
    await state.clear()
...
async def pay_withdraw_inline(call: CallbackQuery):
    if call.from_user.id != ADMIN_ID: return
    tid = int(call.data.split(":")[1])
    # Здесь можно добавить создание нового чека, если нужно
